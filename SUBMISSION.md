...
> NimHouse is a skill-based mini game house inside Nimiq Pay, bundling seven original games: NimChick (absurd flapper), NimStack (floating blocks, drop precision), NimBullseye (swaying crosshair darts with PERFECT ×2), NimRush (pseudo-3D chicken runner), NimSwat (the angry mustachioed fly), NimSlice (swipe-slicing sausage volleys), and NimHop (endless platform hopper). Every game is 100% player skill — no entry fees, no betting, no chance-based outcomes.
>
